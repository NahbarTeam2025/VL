  ...
  return (
    <section id="faq" className="py-24 bg-bg-secondary relative overflow-hidden" aria-labelledby="faq-title">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <h2 id="faq-title" className="text-[clamp(1.75rem,5vw,3rem)] font-black mb-12 md:mb-16 text-center text-text-head leading-tight">Häufige Fragen</h2>
        
        <div className="space-y-4 md:space-y-6">
          {faqs.map((faq, idx) => (
            <div 
              key={idx} 
              className={`relative group rounded-2xl p-[1px] transition-all duration-500 ${openIndex === idx ? 'bg-gradient-to-r from-[#4FD1FF] to-[#2F80FF]' : 'bg-gradient-to-r from-white/10 to-white/5 [.light_&]:from-black/5 [.light_&]:to-black/5 hover:from-[#4FD1FF]/50 hover:to-[#2F80FF]/50'}`}
            >
              ...
              <div className={`relative bg-bg-primary rounded-2xl overflow-hidden transition-all duration-500 ${openIndex === idx ? 'bg-gradient-to-br from-[#15305b]/80 via-[#0d1f3d]/80 to-[#050b14]/80 [.light_&]:from-blue-50 [.light_&]:via-white [.light_&]:to-blue-50 backdrop-blur-xl border border-[#4FD1FF]/15 [.light_&]:border-blue-200 shadow-[0_2px_10px_rgba(79,209,255,0.05)]' : ''}`}>
                <button 
                  ...
                  id={`faq-question-${idx}`}
                >
                  <span className={`font-bold transition-colors text-base md:text-xl ${openIndex === idx ? 'text-white [.light_&]:text-black [.light_&]:drop-shadow-none drop-shadow-md' : 'text-text-secondary group-hover:text-white [.light_&]:group-hover:text-black'}`}>{faq.q}</span>
                ...
                >
                    <div className="overflow-hidden">
                        <div className={`px-5 md:px-8 pb-6 md:pb-8 pt-0 leading-relaxed text-sm md:text-lg font-light ${openIndex === idx ? 'text-blue-100/90 [.light_&]:text-text-primary' : 'text-text-secondary'}`}>
                            {faq.a}
                        </div>
                    </div>
                </div>
